      ...
      <>
        ...
        <main className={`${styles.main}`}>
            <section className={styles.scrollable}>
                {orders.map((item) => (
                    <OrdersItem key={item._id} onClick={showDetails(item)} showStatus={false} id={item._id} date={item.createdAt} total={0} number={item.number} createdAt={item.createdAt} ingredients={item.ingredients} name={item.name} status={item.status} />
                ))}
            </section>
